clear all; 
load("exp1_data.mat");  %load the data for example 1
...
for i=1:n
   ...
   if i==1
        au(i) = 0;%we set $a_{1u}=0$ since $a_i$ is a null vector
   end
   UGm(i) = data_mip(Xd{i},Zd{i},Ud{i},au(i),L,nl);
end

%% Calculate the control gain
c1 = 0; c2 = 4; %The constants that satisfy Assumption 2 
